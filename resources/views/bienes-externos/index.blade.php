<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-dark-850 overflow-hidden shadow-2xl sm:rounded-2xl border border-dark-800">
                <div class="p-6">
                    ...
                    <form method="GET" action="{{ route('bienes-externos.index') }}" class="mb-8 flex flex-col xl:flex-row gap-4">
                        <!-- Búsqueda -->
                        ...
                        <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            ...
                            <!-- Filtro Departamento + Botones -->
                            <div class="flex gap-2">
                                <div class="flex-1">
                                    <x-select-premium
                                        name="departamento_id"
                                        placeholder="Depto/Servicio"
                                ...
                                @if(request()->anyFilled(['buscar', 'estado_id', 'categoria_bien_id', 'departamento_id']))
                                    <a href="{{ route('bienes-externos.index') }}" class="shrink-0 p-4 bg-rose-500/10 text-rose-400 rounded-2xl hover:bg-rose-500/20 transition-colors shadow-lg shadow-rose-500/5 flex items-center" title="Limpiar Filtros">
                        ...
                        <table class="min-w-full">
                            <thead>
                                <tr class="bg-dark-800/50">
                                    ...
                                    <th scope="col" class="px-6 py-5 text-left text-xs font-bold text-dark-text uppercase tracking-widest">Ubicación (Depto)</th>
                                    ...
                                    <tr class="hover:bg-dark-800/30 transition-all duration-300">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold flex items-center gap-3">
                                            @can('ver bienes externos')
                                                <a href="{{ route('bienes-externos.show', $bien) }}" class="text-sky-400 hover:text-sky-300 transition" title="Ver detalle">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                                </a>
                                            @endcan
                                            @can('editar bienes externos')
                                                <a href="{{ route('bienes-externos.edit', $bien) }}" class="text-amber-400 hover:text-amber-300 transition" title="Editar">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                                </a>
                                            @endcan
                                            @can('eliminar bienes externos')
                                                <button type="button" 
                                                        @click="window.dispatchEvent(new CustomEvent('open-deletion-modal', { detail: { action: '{{ route('bienes-externos.destroy', $bien) }}' } }))" 
                                        ...
                                        <td colspan="6" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center gap-3">
                                                ...
                                                <p class="text-gray-500 dark:text-gray-400 font-medium">
                                                    @if(request()->anyFilled(['buscar', 'estado_id', 'categoria_bien_id', 'departamento_id']))
                                                        No se encontraron bienes externos con los filtros aplicados.
                                                    @else
                                                        No hay bienes externos registrados.
                                                    @endif
                                                </p>
                                            </div>
                                        </td>
    ...
    <x-confirm-deletion 
        title="¿Eliminar Bien Externo?" 
        message="¿Estás seguro de que deseas eliminar este bien externo? Esta acción es irreversible."
    />
