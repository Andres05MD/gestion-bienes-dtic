    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-black text-3xl text-gray-800 dark:text-white leading-tight tracking-tight drop-shadow-md">
                {{ __('Detalle de Distribución') }} <span class="text-brand-lila">#{{ $distribucion->numero_bien }}</span>
            </h2>
            <div class="flex items-center gap-3">
                @can('editar distribuciones')
                    <a href="{{ route('distribuciones-direccion.edit', $distribucion) }}" class="inline-flex items-center px-5 py-2.5 bg-linear-to-r from-brand-lila to-brand-purple border border-transparent rounded-xl font-bold text-xs text-white uppercase tracking-widest hover:brightness-110 active:scale-95 transition-all duration-150 shadow-lg shadow-brand-purple/20">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        {{ __('Editar') }}
                    </a>
                @endcan
                <a href="{{ route('distribuciones-direccion.index') }}" class="text-sm font-bold text-gray-400 hover:text-white transition-colors uppercase tracking-widest text-[10px]">
                    {{ __('← Volver') }}
                </a>
            </div>
        </div>
                                    ...
                                    <div class="space-y-10">
                                        ...
                                        <div class="grid grid-cols-2 gap-8 pt-8 border-t border-white/5">
                                            <div class="space-y-2">
                                                ...
                                                <code class="text-sm font-mono text-brand-lila bg-brand-lila/5 px-3 py-1 rounded-lg border border-brand-lila/10 inline-block">{{ $distribucion->numero_bien }}</code>
                                            </div>
                                            <div class="space-y-2">
                                                <p class="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Marca</p>
                                                <p class="text-base font-bold text-gray-200">{{ $distribucion->marca ?? '—' }}</p>
                                            </div>
                                            <div class="space-y-2">
                                                <p class="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Serial</p>
                                                <p class="text-base font-bold text-gray-200">{{ $distribucion->serial ?? '—' }}</p>
                                            </div>
                                            <div class="space-y-2">
                                                <p class="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Fecha</p>
                                                <p class="text-base font-bold text-gray-200">{{ $distribucion->fecha->format('d/m/Y') }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="bg-dark-850 p-8 rounded-3xl border border-white/5 shadow-xl">
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-lg font-bold text-white">{{ $distribucion->procedencia?->nombre ?? 'N/A' }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="px-6 py-6 rounded-3xl border border-dashed border-white/10 bg-white/2">
                                <div class="space-y-4">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span class="text-xs font-mono text-gray-300">{{ $distribucion->created_at->format('d/m/Y') }}</span>
                                    </div>
                                    <div class="h-px bg-white/5"></div>
                                    <div class="flex items-center justify-between">
                                        <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Registrado por</span>
                                        <span class="text-xs font-bold text-brand-lila">{{ $distribucion->user?->name }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
