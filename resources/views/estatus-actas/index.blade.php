<x-app-layout>
                ...
                <div class="p-6">
                    @if($estatusActas->isEmpty())
                        <div class="flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <h3 class="text-lg font-bold text-white mb-2">No hay estatus de actas registrados</h3>
                            <p class="text-gray-400 text-sm max-w-sm mb-6">Define los posibles estados de las actas de transferencia (Ej: Actas Listas, Pendiente).</p>
                            <a href="{{ route('estatus-actas.create') }}" class="text-brand-lila font-bold hover:text-brand-purple transition-colors text-sm">
                                + Crear primer estatus
                            </a>
                        </div>
                    @else
                            ...
                            <table class="min-w-full">
                                <thead>
                                    <tr class="bg-dark-800/50">
                                        ...
                                        <th scope="col" class="px-6 py-5 text-left text-xs font-bold text-dark-text uppercase tracking-widest">Color / Etiqueta</th>
                                ...
                                <tbody class="divide-y divide-dark-800">
                                    @foreach ($estatusActas as $estatus)
                                        <tr class="hover:bg-dark-800/30 transition-all duration-300">
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold">
                                                <span class="text-white">{{ $estatus->nombre }}</span>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                                <div class="flex items-center gap-2">
                                                    <div class="w-4 h-4 rounded-full" style="background-color: {{ $estatus->color ?? '#a855f7' }}"></div>
                                                    <span class="text-dark-text text-xs uppercase font-bold">{{ $estatus->color ?? 'Por defecto' }}</span>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold flex items-center gap-3">
                                                @can('editar estatus actas')
                                                <a href="{{ route('estatus-actas.edit', $estatus) }}" class="text-amber-400 hover:text-amber-300 transition" title="Editar">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                                </a>
                                                @endcan

                                                @can('eliminar estatus actas')
                                                <button type="button" 
                                                        @click="window.dispatchEvent(new CustomEvent('open-deletion-modal', { detail: { action: '{{ route('estatus-actas.destroy', $estatus) }}' } }))" 
    ...
    <x-confirm-deletion 
        title="¿Eliminar Estatus de Acta?" 
        message="¿Estás seguro de que deseas eliminar este estatus de acta? Esta acción no se puede deshacer."
    />
