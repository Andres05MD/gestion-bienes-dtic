        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-dark-850 shadow-2xl sm:rounded-2xl border border-dark-800 relative z-10">
                <div class="p-6">
                    ...
                    <form method="GET" action="{{ route('mantenimientos.index') }}" class="mb-8 space-y-4">
                        <div class="flex flex-col xl:flex-row gap-4">
                            ...
                            <div class="flex gap-2">
                                ...
                                <a href="{{ route('mantenimientos.index') }}" class="flex-1 xl:flex-none inline-flex items-center justify-center px-6 py-4 bg-rose-500/10 text-rose-400 rounded-2xl hover:bg-rose-500/20 transition-all font-bold text-xs uppercase tracking-widest shadow-lg shadow-rose-500/5" title="Limpiar Filtros">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <table class="min-w-full">
                            <thead>
                                <tr class="bg-dark-800/50">
                                    <th scope="col" class="px-6 py-5 text-left text-xs font-bold text-dark-text uppercase tracking-widest">N° Orden</th>
                                    ...
                                    <th scope="col" class="px-6 py-5 text-left text-xs font-bold text-dark-text uppercase tracking-widest">Movimiento</th>
                                    <th class="px-6 py-4 text-left text-[10px] font-black tracking-[0.2em] text-gray-500 uppercase">Procedencia</th>
                            ...
                            <tbody class="divide-y divide-dark-800">
                                @forelse ($mantenimientosAgrupadas as $nOrdenActa => $grupo)
                                @php
                                ...
                                <tr class="hover:bg-dark-800/30 transition-all duration-300">
                                    <!-- N° Orden Acta -->
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-dark-text font-bold">
                                        {{ $primera->n_orden_acta ?? '—' }}
                                    </td>
                                    <!-- N° Bien Agrupado -->
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-dark-text">
                                        @if($primera->tipo_movimiento === 'entrada')
                                        <span class="text-brand-lila font-bold">ENTRADA</span>
                                        @else
                                        <span class="text-emerald-400 font-bold">DEVOLUCIÓN</span>
                                        @endif
                                        <br>
                                        <span class="text-[10px]">{{ $primera->fecha->format('d/m/Y') }}</span>
                                    </td>

                                    <td class="px-6 py-4 whitespace-normal text-sm text-dark-text min-w-[200px]">
                                        <div class="flex items-center gap-2">
                                            @if($primera->tipo_movimiento === 'entrada')
                                            <span class="font-medium text-white">{{ $primera->procedencia?->nombre ?? 'N/A' }}</span>
                                            @else
                                            <span class="font-medium text-white">{{ $primera->destino?->nombre ?? 'N/A' }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold flex items-center gap-3">
                                        ...
                                        <a href="{{ route('mantenimientos.edit', $primera) }}" class="text-amber-400 hover:text-amber-300 transition" title="Editar Registro">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button type="button"
                                            @click="window.dispatchEvent(new CustomEvent('open-deletion-modal', { detail: { action: '{{ route('mantenimientos.destroy', $primera) }}' } }))"
                                        ...
                                        <!-- Botón Devolver (Solo si es entrada) -->
                                        @if($primera->tipo_movimiento === 'entrada')
