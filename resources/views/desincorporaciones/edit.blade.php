        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('desincorporaciones.update', $desincorporacion) }}" class="space-y-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div class="lg:col-span-2 space-y-8">
                        ...
                        <div x-data="{ activeCard: false }" @click="activeCard = true" @click.outside="activeCard = false" :class="activeCard ? 'z-50' : 'z-40'" class="bg-white dark:bg-dark-850/40 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-white/5 relative group transition-all duration-500">
                            ...
                            <div x-data="{
                                ...
                                globalSearch: '',
                                bienSearch: '',
                                tipoBien: '{{ $desincorporacion->bien_id ? 'dtic' : ($desincorporacion->bien_externo_id ? 'externo' : '') }}',
                                bienId: '{{ $desincorporacion->bien_id ?? $desincorporacion->bien_externo_id ?? '' }}',
                                globalResults: [],
                                bienResults: [],
                                debounceTimer: null,
                                ...
                                <input type="hidden" name="bien_id" id="bien_id" value="{{ old('bien_id', $desincorporacion->bien_id) }}">
                                <input type="hidden" name="bien_externo_id" id="bien_externo_id" value="{{ old('bien_externo_id', $desincorporacion->bien_externo_id) }}">
                            </div>
                        </div>
                        ...
                        <div x-data="{ activeCard: false }" @click="activeCard = true" @click.outside="activeCard = false" :class="activeCard ? 'z-50' : 'z-30'" class="bg-white dark:bg-dark-850/40 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-white/5 relative group transition-all duration-500">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <x-input-premium name="numero_bien" label="Número de Bien" :value="$desincorporacion->numero_bien" required icon="o-hashtag" id="numero_bien" />
                                <x-input-premium name="descripcion" label="Descripción" :value="$desincorporacion->descripcion" required icon="o-document-text" id="descripcion" />
                                <x-input-premium name="serial" label="Serial" :value="$desincorporacion->serial" icon="o-qr-code" id="serial" />
                    ...
                    <div class="space-y-8">
                        <div x-data="{ activeCard: false, procedenciaSeleccionada: '{{ old('procedencia_id', $desincorporacion->procedencia_id ?? $dticId) }}', destinoSeleccionado: '{{ old('destino_id', $desincorporacion->destino_id ?? $dticId) }}' }" @set-selected-procedencia-id.window="procedenciaSeleccionada = $event.detail" @click="activeCard = true" @click.outside="activeCard = false" :class="activeCard ? 'z-50' : 'z-40'" class="bg-white dark:bg-dark-850/40 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-white/5 relative transition-all duration-300">
                            <div class="flex items-center gap-3 mb-8">
                                <div class="w-10 h-10 bg-brand-purple/10 rounded-xl flex items-center justify-center">
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div x-show="procedenciaSeleccionada == {{ $dticId }}" x-transition>
                                        <x-select-premium
                                            ...
                                            :value="old('area_procedencia_id', $desincorporacion->bien?->area_id)"
                                            :required="false" />
                                    </div>
                                </div>
                        ...
                        <div x-data="{ activeCard: false }" @click="activeCard = true" @click.outside="activeCard = false" :class="activeCard ? 'z-50' : 'z-30'" class="bg-white dark:bg-dark-850/40 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-white/5 relative transition-all duration-300">
                            ...
                            <div class="space-y-6">
                                <x-input-premium name="numero_informe" label="N° Informe" :value="$desincorporacion->numero_informe" icon="o-document-text" />
