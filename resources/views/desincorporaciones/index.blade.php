                    ...
                    <div class="overflow-x-auto rounded-xl">
                        <table class="min-w-full">
                            ...
                            <tbody class="divide-y divide-dark-800">
                                @forelse ($desincorporaciones as $desincorporacion)
                                    <tr class="hover:bg-dark-800/30 transition-all duration-300">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-white">{{ $desincorporacion->numero_bien }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ $desincorporacion->descripcion }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-dark-text">{{ $desincorporacion->serial ?? '—' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-dark-text">{{ $desincorporacion->procedencia?->nombre ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-dark-text">{{ $desincorporacion->fecha->format('d/m/Y') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <code class="text-brand-lila bg-brand-lila/5 px-2 py-0.5 rounded-md font-mono text-xs border border-brand-lila/10">{{ $desincorporacion->numero_informe }}</code>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold flex items-center gap-3">
                                            @can('ver desincorporaciones')
                                                <a href="{{ route('desincorporaciones.show', $desincorporacion) }}" class="text-sky-400 hover:text-sky-300 transition" title="Ver detalle">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                                </a>
                                            @endcan
                                            @can('editar desincorporaciones')
                                                <a href="{{ route('desincorporaciones.edit', $desincorporacion) }}" class="text-amber-400 hover:text-amber-300 transition" title="Editar">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                                </a>
                                            @endcan
                                            @can('eliminar desincorporaciones')
                                                <button type="button"
                                                        @click="window.dispatchEvent(new CustomEvent('open-deletion-modal', { detail: { action: '{{ route('desincorporaciones.destroy', $desincorporacion) }}' } }))"
                                    ...
                                    <tr>
                                        <td colspan="8" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center gap-3">
                                                <svg class="w-12 h-12 text-dark-800" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
