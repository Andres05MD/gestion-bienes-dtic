...
{
    /**
     * Ejecuta la migración — Tabla de Transferencias Internas.
     */
    public function up(): void
    {
        Schema::create('transferencias_internas', function (Blueprint $table) {
            $table->id();
            $table->string('numero_bien');
            $table->string('descripcion');
            $table->string('serial')->nullable();
            $table->foreignId('procedencia_id')->constrained('departamentos')->cascadeOnDelete();
            $table->foreignId('destino_id')->constrained('departamentos')->cascadeOnDelete();
            $table->date('fecha');
    ...
    {
        Schema::dropIfExists('transferencias_internas');
    }
};
